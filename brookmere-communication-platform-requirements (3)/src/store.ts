  ...
  return {
    users: {
      [adminId]: {
        ...
        role: 'admin',
        createdAt: 1704067200000, // 2024-01-01 fixed timestamp
...
export function loadState(): AppState {
  try {
    ...
    if (!s.users[adminId]) {
      ...
      // Always reset admin password hash in case it differs
...
export async function loadFromFirebase(): Promise<AppState | null> {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      if (!data.users[adminId]) {
        data.users[adminId] = getDefaultState().users[adminId];
      } else {
        data.users[adminId].passwordHash = hashPassword('6668');
        data.users[adminId].role = 'admin';
      }
      ...
      if (!data.deviceAccountCooldown) data.deviceAccountCooldown = {};
      return data;
    }

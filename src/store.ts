...
export function onFirebaseReady(cb: () => void) {
  if (firebaseEnabled && firebaseDb) { cb(); }
  else { firebaseReadyCallbacks.push(cb); }
}
...
export function setupFirebase(config: {
  ...
  try {
    ...
    if (db) {
      firebaseDb = db; firebaseEnabled = true;
...
export function isFirebaseEnabled() { return firebaseEnabled && firebaseDb !== null; }
...
function ensureAdminExists(s: AppState): AppState {
  ...
  if (!s.directMessages) s.directMessages = {};
  if (!s.dmConversations) s.dmConversations = {};
  return s;
}
...
export function loadState(): AppState {
  try {
    ...
    if (!raw) { const s = getDefaultState(); localStorage.setItem(LOCAL_KEY, JSON.stringify(s)); return s; }
    return ensureAdminExists(JSON.parse(raw) as AppState);
  } catch {
    const s = getDefaultState(); localStorage.setItem(LOCAL_KEY, JSON.stringify(s)); return s;
  }
}
...
export function saveState(state: AppState): void {
  try {
    ...
    if (firebaseEnabled && firebaseDb) syncToFirebase(state);
...
export function subscribeToFirebase(callback: (state: AppState) => void): () => void {
  ...
  try {
    ...
    }, (error) => { console.warn('Firebase subscription error:', error); });
    firebaseListeners.push(unsubscribe);
    return unsubscribe;
  } catch (e) { console.warn('Firebase subscribe failed:', e); return () => {}; }
}

export function onStateChange(callback: (state: AppState) => void): () => void {
  if (!CHANNEL) return () => {};
  const handler = (event: MessageEvent) => {
    if (event.data?.type === 'STATE_UPDATE') callback(event.data.state as AppState);
  };
...
export function getDeviceId(): string {
  ...
  if (!deviceId) { deviceId = 'device_' + generateId(); localStorage.setItem('brookmere_device_id', deviceId); }
  return deviceId;
}

export function getCurrentSessionKey(): string { return 'brookmere_session_' + getDeviceId(); }
export function getStoredSession(): string | null { return localStorage.getItem(getCurrentSessionKey()); }
export function storeSession(userId: string): void { localStorage.setItem(getCurrentSessionKey(), userId); }
export function clearSession(): void { localStorage.removeItem(getCurrentSessionKey()); }

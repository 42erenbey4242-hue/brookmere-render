...
export default function SettingsPanel({ user, onChangeName, onUpdateProfile, onLogout, onClose }: Props) {
  ...
  const handleSaveProfile = async () => {
    ...
    setProfileSuccess('');
    const avatarIsGif = !!(
      avatarPreview?.startsWith('data:image/gif') ||
      (user.avatarIsGif && avatarPreview && !avatarPreview.startsWith('data:'))
    );
    const bannerIsGif = !!(
      bannerPreview?.startsWith('data:image/gif') ||
      (user.bannerIsGif && bannerPreview && !bannerPreview.startsWith('data:'))
    );
    const result = await onUpdateProfile({ avatarUrl: avatarPreview, avatarIsGif, bannerUrl: bannerPreview, bannerIsGif, bio });
    setProfileSaving(false);
    if (result.success) {
      setProfileSuccess('Profil kaydedildi!');
      setTimeout(() => setProfileSuccess(''), 3000);
    } else {
      setProfileSuccess(`❌ ${result.error || 'Hata oluştu.'}`);
    }
  };
        ...
        <div className="flex-1 p-8 overflow-y-auto">
          {activeTab === 'profile' && (
            <div>
              ...
              >{profileSaving ? '⏳ Yükleniyor...' : 'Değişiklikleri Kaydet'}</button>
            </div>
          )}

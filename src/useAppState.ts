...
import { uploadGifToImgur } from './gifUpload';
...
export function useAppState() {
  ...
  const updateProfile = useCallback(async (userId: string, updates: { avatarUrl?: string | null; avatarIsGif?: boolean; bannerUrl?: string | null; bannerIsGif?: boolean; bio?: string }): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      // GIF avatar → Imgur'a yükle
      if (finalUpdates.avatarUrl?.startsWith('data:image/gif')) {
        const imgurUrl = await uploadGifToImgur(finalUpdates.avatarUrl);
        finalUpdates.avatarUrl = imgurUrl;
        finalUpdates.avatarIsGif = true;
      }
      // GIF banner → Imgur'a yükle
      if (finalUpdates.bannerUrl?.startsWith('data:image/gif')) {
        const imgurUrl = await uploadGifToImgur(finalUpdates.bannerUrl);
        finalUpdates.bannerUrl = imgurUrl;
        finalUpdates.bannerIsGif = true;
      }
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      console.warn('Imgur upload failed:', msg);
      return { success: false, error: `GIF yüklenemedi: ${msg}` };
    }

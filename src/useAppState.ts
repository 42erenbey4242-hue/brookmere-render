...
export function useAppState() {
  ...
  useEffect(() => {
    const unsub = onStateChange((newState) => { setStateInternal(newState); stateRef.current = newState; });
    return unsub;
  }, []);
  ...
  useEffect(() => {
    ...
    onFirebaseReady(async () => {
      ...
      if (fbState) { setStateInternal(fbState); stateRef.current = fbState; }
      unsubFirebase = subscribeToFirebase((newState) => { setStateInternal(newState); stateRef.current = newState; });
    });
    return () => { if (unsubFirebase) unsubFirebase(); };
  }, []);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setStateInternal(prev => {
        ...
        const next = { ...prev, users: { ...prev.users, [userId]: { ...user, online: true, lastSeen: Date.now() } } };
        stateRef.current = next; saveState(next); return next;
      });
    }, 5000);
  ...
  useEffect(() => {
    const handleUnload = () => {
      ...
      if (s.users[userId]) {
        ...
        if (isFirebaseEnabled()) saveState(s);
      }
    };
    ...
    const interval = setInterval(() => {
      ...
      setStateInternal(prev => {
        ...
        Object.values(newUsers).forEach(user => {
          ...
          if (user.online && now - user.lastSeen > 15000) { newUsers[user.id] = { ...user, online: false }; changed = true; }
        });
  ...
  const register = useCallback((username: string, password: string): { success: boolean; error?: string } => {
    ...
    const newUser: User = {
      id: userId, username: username.trim(), passwordHash: hashPassword(password),
      role: 'citizen', createdAt: Date.now(), lastNameChangeAt: null,
      lastAccountSwitchAt: null, isBanned: false, online: true, lastSeen: Date.now(),
      avatarUrl: null, bannerUrl: null, bio: '',
    };
    s.users[userId] = newUser;
    saveState(s); setStateInternal({ ...s }); stateRef.current = { ...s };
    storeSession(userId); currentUserIdRef.current = userId;
    return { success: true };
  }, []);
  ...
  const loginAdmin = useCallback((username: string, password: string): { success: boolean; error?: string } => {
    ...
    admin.online = true; admin.lastSeen = Date.now(); s.users[adminId] = admin;
    saveState(s); setStateInternal({ ...s }); stateRef.current = { ...s };
    storeSession(adminId); currentUserIdRef.current = adminId;
    return { success: true };
  }, []);
  ...
  const autoLogin = useCallback((): boolean => {
    ...
    if (user.isBanned) { clearSession(); return false; }
    user.online = true; user.lastSeen = Date.now(); s.users[uid] = user;
    saveState(s); setStateInternal({ ...s }); stateRef.current = { ...s };
    currentUserIdRef.current = uid; return true;
  }, []);
  ...
  const logout = useCallback((deleteAccount: boolean = false) => {
    ...
    const s = loadState();
    if (s.users[uid]) {
      s.users[uid].online = false; s.users[uid].lastSeen = Date.now();
      if (deleteAccount) s.deviceAccountCooldown[getDeviceId()] = Date.now();
    }
    saveState(s); setStateInternal({ ...s }); stateRef.current = { ...s };
    clearSession(); currentUserIdRef.current = null;
  }, []);

  const markOnline = useCallback((userId: string) => { currentUserIdRef.current = userId; }, []);
  ...
  const changeName = useCallback((userId: string, newName: string): { success: boolean; error?: string } => {
    ...
    if (user.role !== 'admin') s.users[userId].lastNameChangeAt = Date.now();
    saveState(s); setStateInternal({ ...s }); stateRef.current = { ...s };
    return { success: true };
  }, []);
  ...
  const banUser = useCallback((userId: string) => {
    setState(prev => {
      ...
      // DM'leri de temizle
      const newDMs = Object.fromEntries(
        Object.entries(prev.directMessages).filter(([, dm]) => dm.senderId !== userId && dm.receiverId !== userId)
      );
      const newConvs = Object.fromEntries(
        Object.entries(prev.dmConversations).filter(([, conv]) => !conv.participants.includes(userId))
      );
      return { ...prev, users: newUsers, directMessages: newDMs, dmConversations: newConvs };
    });
  }, [setState]);

  const timeoutUser = useCallback((userId: string, minutes: number) => {
    setState(prev => ({ ...prev, users: { ...prev.users, [userId]: { ...prev.users[userId], timeoutUntil: Date.now() + minutes * 60 * 1000 } } }));
  }, [setState]);

  const removeTimeout = useCallback((userId: string) => {
    setState(prev => ({ ...prev, users: { ...prev.users, [userId]: { ...prev.users[userId], timeoutUntil: null } } }));
  }, [setState]);
  ...
  const createServer = useCallback((name: string): Server => {
    ...
    setState(prev => ({ ...prev, servers: { ...prev.servers, [server.id]: server } }));
    return server;
  }, [setState]);

  const updateServer = useCallback((serverId: string, updates: Partial<Server>) => {
    setState(prev => ({ ...prev, servers: { ...prev.servers, [serverId]: { ...prev.servers[serverId], ...updates } } }));
  }, [setState]);

  const deleteServer = useCallback((serverId: string) => {
    setState(prev => {
      const newServers = { ...prev.servers }; delete newServers[serverId];
      const newChannels = Object.fromEntries(Object.entries(prev.channels).filter(([, c]) => c.serverId !== serverId));
      const newCategories = Object.fromEntries(Object.entries(prev.categories).filter(([, c]) => c.serverId !== serverId));
  ...
  const createCategory = useCallback((serverId: string, name: string): Category => {
    const s = loadState();
    const existingOrders = Object.values(s.categories).filter(c => c.serverId === serverId).map(c => c.order);
    const maxOrder = existingOrders.length > 0 ? Math.max(...existingOrders) : -1;
    const category: Category = { id: 'cat_' + generateId(), serverId, name, order: maxOrder + 1 };
    setState(prev => ({ ...prev, categories: { ...prev.categories, [category.id]: category } }));
    return category;
  }, [setState]);

  const updateCategory = useCallback((categoryId: string, updates: Partial<Category>) => {
    setState(prev => ({ ...prev, categories: { ...prev.categories, [categoryId]: { ...prev.categories[categoryId], ...updates } } }));
  }, [setState]);

  const deleteCategory = useCallback((categoryId: string) => {
    setState(prev => {
      const newCategories = { ...prev.categories }; delete newCategories[categoryId];
      const newChannels = { ...prev.channels };
      Object.values(newChannels).forEach(ch => { if (ch.categoryId === categoryId) newChannels[ch.id] = { ...ch, categoryId: null }; });
  ...
  const reorderCategories = useCallback((updates: Array<{ id: string; order: number }>) => {
    setState(prev => {
      ...
      updates.forEach(({ id, order }) => { if (newCategories[id]) newCategories[id] = { ...newCategories[id], order }; });
  ...
  const createChannel = useCallback((serverId: string, categoryId: string | null, name: string): Channel => {
    const s = loadState();
    const existingOrders = Object.values(s.channels).filter(c => c.serverId === serverId).map(c => c.order);
    const maxOrder = existingOrders.length > 0 ? Math.max(...existingOrders) : -1;
    const channel: Channel = { id: 'ch_' + generateId(), serverId, categoryId, name, order: maxOrder + 1, permission: 'everyone' };
  ...
  const updateChannel = useCallback((channelId: string, updates: Partial<Channel>) => {
    setState(prev => ({ ...prev, channels: { ...prev.channels, [channelId]: { ...prev.channels[channelId], ...updates } } }));
  }, [setState]);
  ...
  const deleteChannel = useCallback((channelId: string) => {
    setState(prev => { const newChannels = { ...prev.channels }; delete newChannels[channelId]; return { ...prev, channels: newChannels }; });
  }, [setState]);
  ...
  const sendMessage = useCallback((channelId: string, userId: string, username: string, content: string): { success: boolean; error?: string } => {
    const s = loadState();
    const user = s.users[userId]; const channel = s.channels[channelId];
    ...
    const message: Message = { id: 'msg_' + generateId(), channelId, userId, username, content: content.trim(), createdAt: Date.now() };
    setState(prev => ({ ...prev, messages: { ...prev.messages, [message.id]: message } }));
    return { success: true };
  }, [setState]);
  ...
  const sendDirectMessage = useCallback((fromUserId: string, toUserId: string, content: string): { success: boolean; error?: string } => {
    const s = loadState();
    const fromUser = s.users[fromUserId];
    if (!fromUser) return { success: false, error: 'Hata.' };
    if (fromUser.isBanned) return { success: false, error: 'Banlandınız.' };
    if (fromUser.timeoutUntil && Date.now() < fromUser.timeoutUntil) return { success: false, error: 'Susturuldunuz.' };
    ...
    const conv: DMConversation = {
      id: convId, participants: [fromUserId, toUserId] as [string, string],
      lastMessageAt: Date.now(),
    };
    setState(prev => ({
      ...prev,
      directMessages: { ...prev.directMessages, [dm.id]: dm },
      dmConversations: { ...prev.dmConversations, [convId]: conv },
    }));
    return { success: true };
  }, [setState]);

  const getOrCreateConversation = useCallback((userId1: string, userId2: string): string => {
    const convId = getConversationId(userId1, userId2);
    setState(prev => {
      if (prev.dmConversations[convId]) return prev;
      const conv: DMConversation = {
        id: convId, participants: [userId1, userId2] as [string, string],
        lastMessageAt: Date.now(),
      };
      return { ...prev, dmConversations: { ...prev.dmConversations, [convId]: conv } };
    });
    return convId;
  }, [setState]);
  ...
  return {
    ...
    sendMessage, sendDirectMessage, getOrCreateConversation,
  };
}

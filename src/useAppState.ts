...
export function useAppState() {
  ...
  // Cross-tab sync (aynı cihaz, farklı sekmeler)
  useEffect(() => {
  ...
  // Firebase real-time subscription — Firebase hazır olduğunda başlat
  useEffect(() => {
    let unsubFirebase: (() => void) | null = null;

    const startFirebase = async () => {
      // Önce Firebase'den güncel veriyi çek
      const fbState = await loadFromFirebase();
      if (fbState) {
        setStateInternal(fbState);
        stateRef.current = fbState;
      }

      // Sonra gerçek zamanlı değişikliklere abone ol
      unsubFirebase = subscribeToFirebase((newState) => {
        setStateInternal(newState);
        stateRef.current = newState;
      });
    };

    // Firebase zaten hazırsa hemen başlat, değilse hazır olunca başlat
    onFirebaseReady(() => {
      startFirebase();
    });
  ...
  // Heartbeat — kullanıcının online durumunu canlı tut
  ...
  // Diğer kullanıcıların heartbeat'ini kontrol et — 15 sn cevap gelmezse offline say
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      setStateInternal(prev => {
  ...
  const banUser = useCallback((userId: string, reason?: string) => {
    setState(prev => ({
      ...prev,
      users: { ...prev.users, [userId]: { ...prev.users[userId], isBanned: true, banReason: reason || 'Admin tarafından banlandı.' } }
    }));
  }, [setState]);
  ...
  const unbanUser = useCallback((userId: string) => {
    setState(prev => ({
      ...prev,
      users: { ...prev.users, [userId]: { ...prev.users[userId], isBanned: false, banReason: undefined } }
    }));
  }, [setState]);
  ...
  const timeoutUser = useCallback((userId: string, minutes: number) => {
    setState(prev => ({
      ...prev,
      users: { ...prev.users, [userId]: { ...prev.users[userId], timeoutUntil: Date.now() + minutes * 60 * 1000 } }
    }));
  }, [setState]);
  ...
  const removeTimeout = useCallback((userId: string) => {
    setState(prev => ({
      ...prev,
      users: { ...prev.users, [userId]: { ...prev.users[userId], timeoutUntil: null } }
    }));
  }, [setState]);
  ...
  const updateServer = useCallback((serverId: string, updates: Partial<Server>) => {
    setState(prev => ({
      ...prev,
      servers: { ...prev.servers, [serverId]: { ...prev.servers[serverId], ...updates } }
    }));
  }, [setState]);
  ...
  const updateCategory = useCallback((categoryId: string, updates: Partial<Category>) => {
    setState(prev => ({
      ...prev,
      categories: { ...prev.categories, [categoryId]: { ...prev.categories[categoryId], ...updates } }
    }));
  }, [setState]);
  ...
  const deleteCategory = useCallback((categoryId: string) => {
    setState(prev => {
      ...
      Object.values(newChannels).forEach(ch => {
        if (ch.categoryId === categoryId) newChannels[ch.id] = { ...ch, categoryId: null };
      });
  ...
  const updateChannel = useCallback((channelId: string, updates: Partial<Channel>) => {
    setState(prev => ({
      ...prev,
      channels: { ...prev.channels, [channelId]: { ...prev.channels[channelId], ...updates } }
    }));
  }, [setState]);
  ...
  return {
    ...
    changeName, banUser, unbanUser, timeoutUser, removeTimeout,

...
export default function App() {
  ...
  // Initialize Firebase: önce Render env değişkenlerini dene, sonra localStorage
  useEffect(() => {
    // 1. Render ortam değişkenlerinden otomatik bağlan (deploy'da her zaman çalışır)
    const envConfig = getEnvFirebaseConfig();
    if (envConfig) {
      const ok = setupFirebase(envConfig);
      if (ok) {
    ...
    // 2. Yoksa localStorage'dan önceki config'i dene

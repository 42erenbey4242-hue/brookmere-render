...
interface Props {
  ...
  onSendMessage: (content: string) => { success: boolean; error?: string };
}
...
export default function ChatArea({ channel, messages, currentUser, users, onSendMessage }: Props) {
          ...
          return (
            <div key={groupIdx} className="group hover:bg-[#2e3035] px-2 py-0.5 rounded -mx-2">
              <div className="flex gap-4">
                {/* Avatar */}
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0 mt-0.5 ${isAdmin ? 'bg-[#f5a623]' : 'bg-[#5865f2]'}`}>
                  {(sender?.username || firstMsg.username).charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2 mb-0.5">
                    <span className={`font-semibold text-sm ${isAdmin ? 'text-[#f5a623]' : 'text-white'}`}>
                      {sender?.username || firstMsg.username}
                      {isAdmin && ' 👑'}
                    </span>
